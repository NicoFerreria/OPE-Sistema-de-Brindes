...
namespace Cadastros.Controles
{
    public class ClienteControle
    {
        ...
        public void GravarCliente(ClienteModelo cliente)
        {
            try
            {
                if (ValidaDados(cliente) == false) return;
                ClienteDao dao = new ClienteDao();                
                if (dao.ContemCliente(cliente) == false)
                {
                    dao.Create(cliente);
                }
                else
        ...
        public void ExcluirCliente(ClienteModelo cliente)
        {
            try
            { 
                if (cliente.ClienteId <= 0) throw new System.Exception("Não foi possível excluir cliente! Id do cliente inválido.");
                ClienteDao dao = new ClienteDao();
                dao.Delete(cliente);
        ...
        /// <returns></returns>
        public List<ClienteModelo> BuscaCliente(string criterio,bool excluidos=false)
        {
            try
            {
                ClienteDao dao = new ClienteDao();
        ...
        public ClienteModelo GetCliente(long id)
        {
            try
            {
                if (id <= 0) return new ClienteModelo();
                
                ClienteDao dao = new ClienteDao();
                return dao.SelectClietePorId(id);
